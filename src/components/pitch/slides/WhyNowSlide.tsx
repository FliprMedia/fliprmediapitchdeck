...
import { TrendingUp, Zap, ShoppingBag, Radio, Target, Users, Globe } from "lucide-react";
...
const themes = [
  ...
  { icon: Radio, title: "Every Major Platform Is Investing", desc: "TikTok, YouTube, eBay, Whatnot, Amazon — all pouring capital in" },
  { icon: Target, title: "Advertisers Have No Way In", desc: "Brands want to reach commerce creators, but no media channel serves them" },
  { icon: Globe, title: "The Category Has No Voice", desc: "No TechCrunch, no Bloomberg, no PitchBook for this market. That's the gap." },
];
...
const WhyNowSlide = () => (
  <SlideLayout>
    ...
    <svg className="absolute inset-0 w-full h-full opacity-[0.025]" viewBox="0 0 1920 1080">
      {[0, 1, 2, 3].map(i => (
        <line key={i} x1={`${200 + i * 350}`} y1="1080" x2={`${350 + i * 300}`} y2="80" stroke="hsl(var(--primary))" strokeWidth="1" />
      ))}
    </svg>
    ...
    <div className="relative z-10 flex flex-col h-full px-24 py-14">
      ...
      <h2 className="slide-headline mb-2 opacity-0 animate-fade-up" style={{ animationDelay: '0.1s' }}>
        The window is open <span className="font-display italic text-[54px] text-primary">right now.</span>
      </h2>
      ...
      <div className="flex flex-1 gap-7 min-h-0">
        ...
        <div className="w-[620px] flex flex-col gap-4">
          <div className="glass-card-elevated p-6 flex-1 opacity-0 animate-scale-in" style={{ animationDelay: '0.2s' }}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-[14px] font-mono-brand text-muted-foreground tracking-[0.15em] uppercase">US Livestream Ecommerce ($B)</span>
            ...
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={marketData} margin={{ top: 5, right: 5, bottom: 0, left: -10 }}>
                  ...
                  <XAxis dataKey="year" stroke="hsl(240 5% 45%)" fontSize={12} tickLine={false} fontFamily="JetBrains Mono" />
                  <YAxis stroke="hsl(240 5% 45%)" fontSize={12} tickLine={false} axisLine={false} tickFormatter={v => `$${v}B`} fontFamily="JetBrains Mono" />
            ...
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <span className="text-[16px] text-primary font-bold stat-value">$14.64B</span>
              <span className="text-[15px] text-muted-foreground">US livestream ecommerce in 2025</span>
            </div>
          </div>

          {/* Timeline */}
          <div className="glass-card p-5 opacity-0 animate-fade-up" style={{ animationDelay: '0.4s' }}>
            <div className="text-[14px] font-mono-brand text-muted-foreground tracking-[0.15em] uppercase mb-3">Evolution</div>
            <div className="flex items-center gap-0">
              {evolution.map((t, i) => (
                <div key={t.era} className="flex items-center flex-1">
                  <div className="flex flex-col items-center gap-1.5 flex-1">
                    <div className={`w-3.5 h-3.5 rounded-full ${t.active ? 'bg-primary ring-4 ring-primary/20' : 'bg-muted-foreground/20 border border-muted-foreground/30'}`} />
                    <span className={`text-[15px] font-bold ${t.active ? 'text-primary' : 'text-muted-foreground'}`}>{t.era}</span>
                    <span className={`text-[15px] ${t.active ? 'text-primary/80 font-semibold' : 'text-muted-foreground'}`}>{t.label}</span>
                  </div>
          ...
          <div className="flex gap-2.5 opacity-0 animate-fade-up" style={{ animationDelay: '0.5s' }}>
            {["Commerce", "Content", "Creators", "Community"].map((b, i) => (
              <div key={b} className={`glass-card px-4 py-2.5 flex-1 text-center border-${i % 2 === 0 ? 'primary' : 'accent'}/12`}>
                <span className={`text-[15px] font-bold text-${i % 2 === 0 ? 'primary' : 'accent'}`}>{b}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-3">
          {themes.map((theme, i) => (
            <div key={theme.title} className="glass-card p-4 flex items-start gap-3.5 opacity-0 animate-fade-up group hover:border-primary/20 transition-all"
              style={{ animationDelay: `${0.25 + i * 0.06}s` }}>
              <div className="w-10 h-10 rounded-xl bg-primary/8 border border-primary/12 flex items-center justify-center shrink-0 group-hover:bg-primary/12 transition-colors">
                <theme.icon className="w-5 h-5 text-primary" />
              </div>
              <div>
                <h3 className="text-[16px] font-bold text-foreground mb-0.5">{theme.title}</h3>
                <p className="text-[15px] text-muted-foreground leading-relaxed">{theme.desc}</p>
              </div>
            </div>
          ))}
        </div>
