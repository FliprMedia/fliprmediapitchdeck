  ...
  <SlideLayout>
    ...
    <div className="relative z-10 flex flex-col h-full px-24 py-14">
      ...
      <h2 className="slide-headline mb-2 opacity-0 animate-fade-up" style={{ animationDelay: '0.1s' }}>
        Media revenue first. <span className="font-display italic text-[54px] text-primary">SaaS-like margins next.</span>
      </h2>
      ...
      <div className="flex flex-1 gap-7 min-h-0">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="glass-card p-5 flex-1 opacity-0 animate-fade-up" style={{ animationDelay: '0.2s' }}>
            <div className="text-[15px] font-semibold text-muted-foreground uppercase tracking-wider mb-3">3-Year Revenue Scenarios ($K)</div>
            <div className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={scenarioData} barCategoryGap="25%">
                  <XAxis dataKey="year" stroke="hsl(240 5% 45%)" fontSize={13} tickLine={false} />
                  <YAxis stroke="hsl(240 5% 45%)" fontSize={12} tickLine={false} axisLine={false} tickFormatter={v => v >= 1000 ? `$${v/1000}M` : `$${v}K`} />
            ...
            <div className="flex items-center justify-center gap-6 mt-2">
              {[
                ...
                <div key={l.label} className="flex items-center gap-2">
                  <div className="w-3.5 h-3.5 rounded-sm" style={{ backgroundColor: l.color }} />
                  <span className="text-[15px] text-muted-foreground font-medium">{l.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-card p-4 opacity-0 animate-fade-up" style={{ animationDelay: '0.4s' }}>
            <span className="text-[15px] font-bold text-muted-foreground uppercase tracking-wider block mb-2">Revenue Streams Modeled</span>
            <div className="flex flex-wrap gap-2">
              {streams.map(s => (
                <div key={s} className="px-3 py-1.5 rounded-full bg-muted/30 text-[15px] text-foreground font-medium">{s}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="w-[500px] flex flex-col gap-4">
          <div className="grid grid-cols-3 gap-2.5 opacity-0 animate-fade-up" style={{ animationDelay: '0.3s' }}>
            {[
              ...
              <div key={s.label} className="glass-card p-3.5 text-center">
                <div className="text-[15px] font-semibold text-muted-foreground uppercase mb-1">{s.label}</div>
                <div className="text-[24px] font-black mb-0.5" style={{ color: s.color }}>{s.yr3}</div>
                <div className="text-[14px] text-muted-foreground font-medium">Year 3</div>
                <div className="text-[14px] text-muted-foreground mt-1">{s.desc}</div>
              </div>
            ))}
          </div>
          ...
          <div className="glass-card p-5 border-primary/20 opacity-0 animate-fade-up" style={{ animationDelay: '0.5s' }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[15px] font-bold text-foreground">Long-Term Vision</span>
            </div>
            <p className="text-[15px] text-muted-foreground leading-relaxed">
              "Flipr.Media is being built as a <span className="text-primary font-semibold">category platform with future optionality</span> across media, sponsorship, intelligence, and industry infrastructure — compounding into recurring, high-margin revenue."
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2.5 opacity-0 animate-fade-up" style={{ animationDelay: '0.6s' }}>
            {[
              ...
              <div key={m.label} className="glass-card p-3.5 text-center border-accent/10">
                <div className="text-[15px] text-muted-foreground mb-0.5 font-medium">{m.label}</div>
                <div className="text-[20px] font-black text-accent">{m.range}</div>
                <div className="text-[14px] text-muted-foreground">Revenue Multiple</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 opacity-0 animate-fade-up" style={{ animationDelay: '0.7s' }}>
        <div className="flex items-center gap-3">
          <span className="text-[15px] font-bold text-muted-foreground uppercase tracking-wider shrink-0">Strategic Value Drivers</span>
          <div className="h-px flex-1 bg-border/30" />
        </div>
        <div className="flex items-center justify-between mt-3">
          {valueDrivers.map((d, i) => (
            <div key={d.label} className="flex items-center gap-1.5">
              {i > 0 && <ArrowRight className="w-3.5 h-3.5 text-muted-foreground/70 mr-2" />}
              <div className="glass-card px-3.5 py-2 flex items-center gap-2">
